...
export default function Workspace() {
  ...
  // Someone new is waiting ΓÇö make sure the admin actually notices.
  useEffect(() => {
  ...
  return (
    <div className="workspace">
      <header className="topbar">
        ...
        <span className="status">{connected ? 'connected' : 'reconnectingΓÇª'}</span>

        <div className="peers">
          {peers.map((p) => (
            <span key={p.clientId} className="peer" style={{ background: p.color }}>
              {p.name}
              {p.role === 'admin' && <span className="peer-crown">Γÿà</span>}
            </span>
          ))}
        </div>
