...
/**
 * REPLAY ΓÇö the last item on the Axlero plan (Week 4).
 *
 ...
 * as it stood after update N ΓÇö not an approximation of it.
 *
...
export default function ReplaySlider({ workspaceId, fetchLogs, onClose }) {
  ...
  // genuine mount-only effect ΓÇö depending on the prop directly would re-fetch
  ...
  // Esc closes, arrows step ΓÇö a scrubber people can drive from the keyboard.
  useEffect(() => {
    const onKey = (e) => {
      ...
      <div className="replay-window">
        ...
        {loading ? (
          <div className="replay-empty">
            ...
            <p className="empty">Loading session historyΓÇª</p>
          </div>
        ) : error ? (
          <div className="replay-empty">
          ...
          <>
              ...
              <div className="replay-stage" ref={stageBoxRef}>
                ...
                <div className="replay-zoom">
                  <button className="zoom-btn" title="Zoom out"
                    onClick={() => setView((v) => ({ ...v, scale: Math.max(0.1, v.scale / 1.25) }))}>ΓêÆ</button>
            ...
            <footer className="replay-controls">
              ...
              <select className="replay-speed" value={speed}
                ...
                {SPEEDS.map((s) => <option key={s} value={s}>{s}├ù</option>)}
              </select>
            </footer>
